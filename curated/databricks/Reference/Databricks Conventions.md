...
- One schema per layer: `standalone_bronze`, `standalone_silver`, `standalone_gold`, and, added 2026-09-17, `pure_bronze`. `pure_silver` now exists too (added 2026-09-18, see [[Pure Silver Pipeline]]); only `pure_gold` remains reserved, not yet created.
...
- Live under `/Users/<email>/ri_standalone/`, mirroring the `standalone_*` schemas — or, added 2026-09-17, under `/Users/<email>/ri_pure/`, mirroring the `pure_*` schemas (see [[Pure Bronze Pipeline]]). A notebook's home folder always mirrors the schema family it builds.
